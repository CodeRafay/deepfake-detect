...
def generate_face_like_pattern(size: int = 224) -> np.ndarray:
    ...
    # Add mouth
    cv2.ellipse(img, (size // 2, size // 2 + 50), (25, 10), 0, 0, 180, (150, 100, 100), 2)
        ...
        if roi.shape[0] > 0 and roi.shape[1] > 0:
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    ni, nj = min(max(i + offset_y, 0), rows-1), min(max(j + offset_x, 0), cols-1)
                    roi[i, j] = roi[ni, nj]
...
def generate_sample_dataset(output_dir: str,
    ...
    for i in tqdm(range(num_train_each), desc="Train real"):
        ...
        cv2.imwrite(str(output_path / 'train' / 'real' / f'real_{i:04d}.jpg'), img)
    
    for i in tqdm(range(num_train_each), desc="Train fake"):
        img = generate_face_like_pattern(image_size)
        fake = add_fake_artifacts(img)
        cv2.imwrite(str(output_path / 'train' / 'fake' / f'fake_{i:04d}.jpg'), fake)
    
    # Generate test data
    print("\nGenerating test data...")
    for i in tqdm(range(num_test_each), desc="Test real"):
        img = generate_face_like_pattern(image_size)
        cv2.imwrite(str(output_path / 'test' / 'real' / f'real_{i:04d}.jpg'), img)
    
    for i in tqdm(range(num_test_each), desc="Test fake"):
        img = generate_face_like_pattern(image_size)
        fake = add_fake_artifacts(img)
        cv2.imwrite(str(output_path / 'test' / 'fake' / f'fake_{i:04d}.jpg'), fake)
